export async function api(path, { method = 'GET', body } = {}) {
  let response;
  try {
    response = await fetch('/api' + path, { method, credentials: 'same-origin', cache: 'no-store', keepalive: path === '/push/test', headers: { 'X-Ahorra-Request': '1', ...(body !== undefined ? { 'Content-Type': 'application/json' } : {}) }, body: body !== undefined ? JSON.stringify(body) : undefined, signal: AbortSignal.timeout(path.startsWith('/mail/') ? 55000 : 30000) });
