...
function describe(item,{t,money,locale},today){
 ...
 switch(item.kind){
  ...
  case 'movement_new':return {icon:p.type==='income'?'up':'down',tone:'later',title:t(`Nuevo movimiento: ${p.reason}`,`New movement: ${p.reason}`),detail:`${p.type==='income'?t('Entrada','Income'):t('Salida','Expense')}: ${money(p.amountCents,p.currency)} ${p.currency}`,view:'movimientos'};
...
export function NotificationCenter({data,today=zonedToday(),failure,onRefresh,onClose,onNavigate,hidden=false}){
 ...
 async function remove(body){if(body.all===true&&!window.confirm(t('¿Eliminar todas las notificaciones internas?','Delete all internal notifications?')))return;setBusy(true);setMessage('');try{await api('/notifications/delete',{method:'POST',body});await onRefresh();}catch(e){setMessage(error(e));}finally{setBusy(false);}}
 return <Modal className="account-drawer notification-drawer" title={t('Notificaciones','Notifications')} subtitle={t('Lo importante de tu dinero, a tiempo.','Your money’s important moments, on time.')} onClose={onClose}>
  <div className="notification-toolbar"><span>{data?.unread||0} {t('sin leer','unread')}</span><div className="notification-toolbar-actions"><button className="text-button" disabled={busy||!data?.unread} onClick={()=>mark({all:true})}>{t('Marcar todas como leídas','Mark all as read')}</button><button className="text-button danger" disabled={busy||!data?.items?.length} onClick={()=>remove({all:true})}><Icon name="trash" size={14}/>{t('Eliminar todas','Delete all')}</button></div></div>
  ...
  {!data?<p className="muted">{t('Cargando avisos…','Loading notices…')}</p>:!data.items.length?<div className="notification-empty"><Icon name="bell" size={38}/><h3>{t('Todo al día.','All caught up.')}</h3><p>{t('Aquí verás tus pagos, alertas de saldo, avances de metas y nuevos avisos del banco.','See payments, balance alerts, goal progress and new bank alerts here.')}</p></div>:<div className="notification-list">{data.items.map(item=>{const content=describe(item,{...i18n,money:hidden?()=> '••••••':i18n.money},today);return <article key={item.id} className={`notification-item ${item.read_at?'':'unread'}`}><span className={`notification-icon ${content.tone}`}><Icon name={content.icon} size={20}/></span><div><h3>{content.title}</h3><p>{content.detail}</p><small>{new Date(item.created_at).toLocaleString(locale,{dateStyle:'short',timeStyle:'short'})}{!item.active&&` · ${t('Aviso anterior','Previous notice')}`}</small><div className="notification-actions"><button className="text-button" onClick={()=>{onClose();onNavigate(content.view);}}>{t('Ver detalle','View details')}</button>{!item.read_at&&<button className="text-button" disabled={busy} onClick={()=>mark({id:item.id})}>{t('Marcar leída','Mark read')}</button>}<button className="text-button danger" disabled={busy} onClick={()=>remove({id:item.id})}><Icon name="trash" size={13}/>{t('Eliminar','Delete')}</button></div></div>{!item.read_at&&<span className="notification-dot" aria-label={t('Sin leer','Unread')}/>}</article>;})}</div>}
  <div className="alert-rules"><h3>{t('Cuándo te avisamos','When we notify you')}</h3><p>{t('Pagos: desde 2 días antes. Presupuesto: al gastar el 80% y cuando quede 10% o menos. Movimientos: al registrar uno nuevo. Meta cercana: cuando falte el 20% o menos. Banco: al detectar un aviso nuevo.','Payments: starting 2 days before. Budget: at 80% spent and when 10% or less remains. Movements: when a new one is recorded. Near goal: when 20% or less remains. Bank: when a new alert is detected.')}</p></div>
  <p className="hint notification-help">{t('Los avisos se guardan en tu cuenta. Si activas Web Push en Ajustes, también pueden llegar aunque Ahorra+ esté cerrada.','Notices are saved to your account. If you enable Web Push in Settings, they can also arrive while Ahorra+ is closed.')}</p>
 </Modal>;
}
