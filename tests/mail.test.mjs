...
test('PostgreSQL: revisión horaria, descuento único, aislamiento, conflictos y pendientes',async()=>{
 const pool=await embeddedPool();try{
  await migrate(pool);const user=randomUUID(),other=randomUUID();for(const id of [user,other]){await pool.query('INSERT INTO ahorra_users(id,email,password_hash,name) VALUES($1,$2,$3,$4)',[id,id+'@test.invalid','hash','Test']);await newWallet(pool,id);}
  ...
  const {rows:[stored]}=await pool.query('SELECT * FROM ahorra_mail_connections WHERE user_id=$1',[user]);assert.notEqual(stored.secret,'abcdabcdabcdabcd');assert.equal(Number(stored.last_uid),10);
  await service.tick();assert.equal(scans,0,'not due until an hour after connecting');
  async function due(){await pool.query("UPDATE ahorra_mail_connections SET next_check=now()-interval '1 second' WHERE user_id=$1",[user]);await service.tick();}
  const old=await readWallet(pool,user);
  inbox=[{uid:11,key:'gmail-11',receivedAt,mail:mail()}];await due();let current=await readWallet(pool,user);assert.equal(current.ledger.movements.length,1);assert.equal(current.ledger.movements[0].amountCents,125000);assert.equal(current.ledger.movements[0].type,'expense');assert.equal(current.revision,old.revision+1);assert.equal((await service.status(user)).counts.applied,1);
  ...
  await due();assert.equal((await readWallet(pool,user)).ledger.movements.length,1,'same email never applied twice');
  ...
  await service.disconnect(user);assert.equal((await service.status(user)).connection,null);assert.equal((await readWallet(pool,user)).ledger.movements.length,3);assert.equal((await pool.query('SELECT * FROM ahorra_mail_connections')).rows.length,0);
